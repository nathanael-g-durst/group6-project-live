...
# Install the development version from GitHub (replace XXXXXXXXXXX with the token generated)
devtools::install_github(repo = "ptds2024/group6-project", subdir = "package", auth_token = "XXXXXXXXXXX")
...
> **Note on `NA` values:**  
> The `NA` values in the output are **normal** and result from the structure of the order book data. Bids and asks are presented in separate rows, with the **bid-related columns** populated for bid entries and the **ask-related columns** populated for ask entries. The `NA` values simply indicate that no data exists in those columns for the respective side of the order book.
